...
本仓库是扣舷 Windows 桌面端的正式代码主线。新对话继续迭代时，先读本文件，再按顺序读取下面的文件，不要从旧的 `type-tracker` 工作区继续开发。
...
- `v1.7.2`：维护能力已并入后续版本，重点是诊断日志、数据备份、更新失败回滚、打包前检查。
- `v1.8.0`：叩舷大更新，重点是浅色手感仪表盘、手感回放、应用强度标签和旧数据继承。
- `v1.9.6`：当前媒体素材快照版，重点是递还今天、日期看板、热力图重构、应用手感构成、图标统一和可复制媒体素材。
- `2026-09-26`：`v2.0` 方向评审，判断周报、应用分类、Android 对齐、数据库迁移和正式发布准备。

这些排期历史上同步到 Super Productivity，旧标题可能仍以 `[交互节律]` 或 `[叩舷]` 开头；下一次任务整理时统一迁移为 `[扣舷]`。

## 关键边界

- 不记录键盘内容、文本内容、鼠标坐标、截图或浏览器页面内容。
- 数据默认保存在本地 SQLite，不做后台上传。
- 手感回放当前是本地规则分析，不调用外部 AI 模型。
